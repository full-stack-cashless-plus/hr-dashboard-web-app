...
export default {
  ...
  computed: {
    ...
    itemsDataTotal() {
      if (this.gridApi) return this.$store.state.area.total;
    },
    paginationPageSize() {
      return 10;
    },
    totalPages() {
      if (this.gridApi) return this.$store.state.area.num_of_pages;
      else return 0;
    },
    currentPage: {
      get() {
  ...
  methods: {
    // Fetch area Data
    fetchItemsData(val) {
      this.$store.dispatch("area/fetchItemsMainReport", val).catch(err => {
        this.$vs.notify({
          title: "Error",
          text: err,
          iconPack: "feather",
    ...
    goToDeleteArea() {
      ...
      // .join(", ");
      // alert(selectedDataStringPresentation); onRemoveSelected
        let upData = new FormData();
      upData.append("areas_ids", selectedDataStringPresentation);
      axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/area/delete/`,{data:upData})
        .then(response => {
