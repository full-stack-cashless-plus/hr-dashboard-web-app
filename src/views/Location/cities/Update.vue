  ...
  <div id="data-list">
    ...
    >
      <form>
        ...
        <vs-button :disabled="city_name == null" type="filled" @click="openConfirm()" class="mb-2"
          >UPDATE</vs-button
        >
      </form>
    </vx-card>
...
export default {
  ...
  methods: {
    openConfirm(){
      ...
      this.$vs.dialog({
        ...
        accept:this.updateCity
      })
      },
    updateCity() {
      this.$vs.loading();
      const formData = new FormData();
      formData.append("name", this.city_name);
     

      axios
        .post(`${process.env.VUE_APP_BASE_URI}/dashboard/city/${this.$route.query.SID}/update/` , formData)
        .then(response => {
  ...
  mounted() {
    ...
    if (this.$vs.rtl) {
      ...
      axios
      .get(
        `${process.env.VUE_APP_BASE_URI}/dashboard/city/${this.$route.query.SID}/`
      )
      .then(response => {
        this.city_name = response.data.data.name;
      });
  },
  created() {
