...
export default {
  ...
  computed: {
    ...
    itemsDataTotal() {
      if (this.gridApi) return this.$store.state.city.total;
    },
    paginationPageSize() {
      return 10;
    },
    totalPages() {
      if (this.gridApi) return this.$store.state.city.num_of_pages;
      else return 0;
    },
    currentPage: {
      get() {
  ...
  methods: {
    // Fetch city Data
    fetchItemsData(val) {
      this.$store.dispatch("city/fetchItemsMainReport", val).catch(err => {
        this.$vs.notify({
          title: "Error",
          text: err,
          iconPack: "feather",
    ...
    goToDeleteCity() {
      ...
      upData.append("cities_ids", selectedDataStringPresentation);
      axios
        .delete(`${process.env.VUE_APP_BASE_URI}/dashboard/city/delete/`,{data:upData})
        .then(response => {
