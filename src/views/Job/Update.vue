        ...
        <div class="vx-row">
          ...
          <div class="vx-col md:w-1/2 sm:w-1/2 w-full mb-2">
            ...
            <v-select
              ...
              :options="departments.map(dept=>(dept.id + ' - ' + dept.department_name + ' - ' + dept.category_name + ' - ' + dept.company_name.title))"
              :clearable="false"
              :dir="$vs.rtl ? 'rtl' : 'ltr'"
              v-model="department"
              class="mb-4 md:mb-0"
...
export default {
  ...
  methods: {
    ...
    updateJob() {
     ...
     //console.log(JSON.stringify(this.department));
      var depts_id = this.department.map(depts=>depts.substring(0,depts.indexOf('-')-1));
      //console.log(depts_id);
  ...
  mounted() {
    ...
    axios
      .get(`${process.env.VUE_APP_BASE_URI}/dashboard/department/all/`)
      .then(response => {
      ...
      .then(response => {
        ...
        this.department = response.data.data.departments_list.map(dept=>(dept.id + ' - ' + dept.department_name + ' - ' + dept.category_name + ' - ' + dept.comapny_info.company_title));  
    // this.department = response.data.data.departments_list.filter(dept => {return dept.id});
    });
  },
  created() {
