...
<template>
  <div id="data-list">
    ...
    <div class="vx-card p-6 vs-con-loading__container" id="div-with-loading">
      ...
      <vs-pagination v-if="show1" :total="totalPages" :max="7" v-model="currentPage" />
    </div>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      columnDefs: [
        ...
        },
        {
         ...
         {
          headerName: "Total Absenteeism Days",
          field: "total_absenteeism_days",
          width: 175,
        },
          {
          headerName: "Penalty Absenteeism",
          field: "penalty_absenteeism",
          width: 175,
        },
          {
          headerName: "Total Latency Time",
          field: "total_latency_time",
          width: 175,
        },
         {
          headerName: "Penalty Latency",
          field: "penalty_latency",
          width: 175,
        },
        {
          headerName: "Total Over Time",
          field: "total_overtime_time",
          width: 175,
        },
      ],
  ...
  watch: {
    city(obj) {
      ...
      axios
        .get(
          `https://hrattendence.gs-group.nl/dashboard/main_report/city/areas/?city=${obj}`
        )
        .then((response) => {
  ...
  computed: {
    itemsData() {
      return this.$store.state.hrReportManagement.itemsVariables;
    },
    ...
    totalPages() {
      if (this.gridApi) return this.$store.state.hrReportManagement.num_of_pages_variables;
      else return 0;
    },
  currentPage: {
      get() {
        if (this.gridApi) return this.items.num_of_pages;
        else return 1;
      },
      set(val) {
       
        this.items.num_of_pages = val;
        // ////console.log('this.items.num_of_pages : ' + this.items.num_of_pages);
        this.fetchItemsData();
  ...
  methods: {
      ...
      this.$store
        .dispatch(
              "hrReportManagement/fetchItemsVariablesReport",
    ...
    resetColFilters(card) {
      ...
      this.items.start_date = this.items.end_date = this.category = this.company = this.items.job_title = this.items.employee_name = this.items.employee_code = this.items.reporting_to = this.items.city = this.city = this.items.area = this.items.area_name = null;
  ...
  mounted() {
    ...
    axios
      .get(`https://hrattendence.gs-group.nl/dashboard/main_report/cities/`)
      .then((response) => {
