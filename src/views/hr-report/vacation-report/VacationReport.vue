    ...
    >
      <form>
        <div class="vx-row">
          ...
          <div class="vx-col md:w-1/4 sm:w-1/2 w-full mb-2">
            <label class="text-sm opacity-75"> <span class="text-primary">*</span>Departments</label>
            <v-select
          ...
          <div class="vx-col md:w-1/4 sm:w-1/2 w-full mb-2" v-if="show">
            <label class="text-sm opacity-75">Annual Pool</label>
            <vs-input
              class="w-full"
              icon-pack="feather"
              icon="icon-hash"
              icon-no-border
              v-model="items.annual_pool"
            />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      columnDefs: [
        ...
        {
          headerName: "Annual Pool",
          field: "annual_pool",
          width: 200,
        },
        {
          headerName: "Casual Pool",
          field: "casual_pool",
          width: 200,
        },
        {
        ...
        {
          headerName: "Remain Annual Pool",
          field: "remain_annual_pool",
          width: 200,
        },
        {
          headerName: "Remain Casual Pool",
          field: "remain_casual_pool",
          width: 200,
        },
      ],
  ...
  computed: {
    itemsData() {
      return this.$store.state.hrReportManagement.itemsVacation;
    },
    paginationPageSize() {
  ...
  methods: {
    addNewFilter() {
      this.$validator.validateAll().then((result) => {
        ...
        if (result) {
          ...
          this.$store
            .dispatch(
              "hrReportManagement/fetchItemsVacationReport",
    ...
    resetColFilters(card) {
      ...
      this.items.from = this.items.to = this.category = this.company = this.items.job_title = this.items.employee_name = this.items.employee_code = this.items.reporting_to = this.items.city = this.city = this.items.area = this.items.status = this.items.annual_pool = null;
  ...
  created() {
    ...
    // this.$store.dispatch("hrReportManagement/fetchItemsVacationReport").catch(err => {
    //   console.error(err);
    // });
  },
};
