...
export default {
  ...
  computed: {
    ...
    itemsDataTotal() {
      return this.$store.state.department.total;
    },
    paginationPageSize() {
      return 10;
    },
    totalPages() {
      if (this.gridApi) return this.$store.state.department.num_of_pages;
      else return 0;
    },
    currentPage: {
      get() {
  ...
  methods: {
    ...
    fetchItemsData(val) {
      this.$store
        .dispatch("department/fetchItemsMainReport", val)
        .catch(err => {
          this.$vs.notify({
            title: "Error",
            text: err,
    ...
    goToDeleteDepartment: function() {
      ...
      upData.append("departments_ids", selectedDataStringPresentation);
      axios
        .delete(
          `${process.env.VUE_APP_BASE_URI}/dashboard/department/delete/`,
          { data: upData }
        )
        .then(response => {
