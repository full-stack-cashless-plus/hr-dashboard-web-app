...
export default {
  ...
  //   });
  // },
  fetchItemsReportingTo({ commit }, params) {
   // alert(params.page + ' , ' + params.manager_name);
    return new Promise((resolve, reject) => {

      axios
        .get(`${process.env.VUE_APP_BASE_URI}/dashboard/reporting_to/?page=${params.page}&manager_name=${params.manager_name}`)
        .then(response => {
          commit("SET_ITEMS", response.data.data);
